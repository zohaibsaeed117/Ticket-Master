...
const AddBusModal: React.FC<AddBusModalProps> = ({ id, button, size }) => {
    ...
    const getData = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/train/get-train/${id}`, {
                headers: {
    ...
    const handleInputChange = (index: number, field: string, value: string) => {
        const updatedCategorys = [...category];
        updatedCategorys[index][field] = value;
        // Automatically set the next row's start value
        if (field === "end" && value) {
            setSeats(value)
            const nextStart = parseInt(value, 10) + 1;
            if (updatedCategorys[index + 1]) {
                updatedCategorys[index + 1].start = nextStart.toString();
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/train/${purpose == "Add" ? "add" : "update"}-train${purpose == "Update" ? `/${id}` : ""}`, {
                    ...
                    <form className="my-8" onSubmit={handleSubmit}>
                        ...
                        <div>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className="w-10">B.No</TableHead>
