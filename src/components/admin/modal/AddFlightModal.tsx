...
const AddFlightModal: React.FC<AddFlightModalProps> = ({ id, button, size }) => {
    ...
    const getData = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/flight/get-flight/${id}`, {
                headers: {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/flight/${purpose == "Add" ? "add" : "update"}-flight${purpose == "Update" ? `/${id}` : ""}`, {
        ...
        toast.success(purpose === "Add" ? "Flight details have been added successfully!" : "Flight details have been updated successfully!");
        console.log({
            title,
            departureCity,
            arrivalCity,
                ...
                <ModalBody className="p-10 overflow-x-auto overflow-y-auto no-scrollbar">
                    <form className="my-8" onSubmit={handleSubmit}>
                        <LabelInputContainer className="mb-4">
                            <Label htmlFor="flightName">Title</Label>
                            <Input
                                id="flightName"
                        ...
                        <LabelInputContainer className="mb-4">
                            ...
                            <Textarea
                                id="description"
                                placeholder="Provide a description of the flight service..."
                                value={description}
                        ...
                        <button
                            ...
                            type="submit"
                        >
                            {purpose} Flight <span className="inline-block transition-transform duration-300 ease-in-out group-hover:translate-x-1">&rarr;</span>
                        </button>
                    </form>
                </ModalBody>
            </Modal>
