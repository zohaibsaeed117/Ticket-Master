  ...
  const renderItems = seats.map((seat, index) => (
    <React.Fragment key={seat.seatNumber}>
      <SeatButton
        ...
        isReserved={seat.reservedBy}
      />
      {/* Insert empty div after every second seat, but not after the last group of seats */}
      {index % 2 === 1 && index < seats.length - 1 && (
  ...
  return (
    <div className="flex justify-center">
      <div className="flex justify-center flex-col items-end">
        ...
        <div className="grid grid-cols-9 gap-2">{renderItems}</div>
      </div>
    </div>
  );
};
