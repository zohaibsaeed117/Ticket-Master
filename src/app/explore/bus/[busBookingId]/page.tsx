...
const BusBookingPage: React.FC<BusBookingPageProps> = ({ params }) => {
    ...
    const [seats, setSeats] = useState<(string | null)[]>([
        "male", null, "male", null, "female", "female", "male", "male", "female", "female",
        "male", "male", "male", null, "female", "male", "female", "male", "female", "male",
        "female", "male", null, "female", "male", "female", "female", "female", "female", "male",
        "male", "female", "female", "female", "male", "female", "female", "male", "female", "male",
        "male", "male", "male", null, "male", "female", "male", "female", "male"
    ])

    const handleSelectedSeatChange = (seatIndex: number) => {
        if (selectedSeats.length === 4) {
            return toast.error("You cannot select more than 4 seats");
        }
        setSelectedSeats((seats) => [...seats, seatIndex]);
    };
    const handleRemoveSelectedSeat = (seatIndex: number) => {
        const tempSeats = selectedSeats.filter(value => value !== seatIndex)
    ...
    const handleSeatSelect = (seatIndex: number, selectedBy: string) => {
        ...
        tempSeats[seatIndex] = selectedBy;
        setSeats(tempSeats)
        ...
        <>
            <div className="p-4 sm:p-8 w-full mx-auto gradient-background flex-wrap rounded-xl flex justify-center md:justify-between items-start sm:items-center">
                <div className="grow sm:grow-0 order-1">
                    <h1 className="text-2xl md:text-2xl lg:text-4xl font-bold">SkyWays</h1>
                    <p className="text-sm md:text-base font-light">Executive Class</p>
                </div>
                <div className="grow order-3 sm:order-2 md:text-xl lg:text-3xl text-xl font-medium flex gap-x-4 items-center justify-center mx-auto">
                    Lahore<MoveRight size={50} className="inline-block" />Islamabad
                </div>
                <div className="flex order-2 sm:order-3 items-center justify-center flex-col">
                    <p className="text-xl md:text-xl lg:text-3xl font-bold mx-auto">Oct 16, 2020</p>
                    <div className="flex items-center justify-center gap-x-2">
                        <p className="text-sm md:text-base lg:text-lg font-light">43 Seats Left</p>
                        <Separator orientation="vertical" className="h-4 sm:h-6 md:h-8 w-[2px] rounded-full" />
                        <p className="text-sm md:text-base lg:text-lg font-light">4:30pm</p>
                    </div>
                </div>
            </div>

            <div className='flex gap-4 my-2 flex-col lg:flex-row '>
                <div className="border rounded-xl border-border bg-card text-card-foreground p-4 sm:p-8  w-full">
                    <h1 className="text-2xl font-medium">Seat Selection</h1>
                    <div className="flex mt-4 gap-4 flex-col lg:flex-row">
                        <div className="border rounded-xl p-4 w-full lg:w-1/2'">
                            <SeatSelector seats={seats} setSeats={setSeats} selectedSeats={selectedSeats} setSelectedSeats={handleSelectedSeatChange} handleSeatSelect={handleSeatSelect} />
                        </div>
